...
static void dtsGetResetGpioInfo(void)
{
	...
	if (of_property_read_u32(node, CHIP_RESET_DEFAULT_VAL_PROPERTY_NAME,
				&default_level) != 0) {
		MR_Info("%s: Failed to get default_level: %s\n",
	...
	if (i4Status < 0) {
		MR_Info("%s: gpio %d request failed, ret = %d\n",
...
enum ReturnStatus resetHif_UpdateSdioHost(void *data)
{
	...
	host = func->card->host;
	if (prSdioHost != host) {
		MR_Info("%s: sdio host is updated as %p\n", __func__, host);
		prSdioHost = host;
		dump_stack();
	}
...
{
	if (!prSdioHost) {
		MR_Info("%s: sdio host is NULL\n", __func__);
		return;
	}
	if (!isSdioAdded) {
		MR_Info("%s: sdio is already removed\n", __func__);
		return;
	}
	prSdioHost->rescan_entered = 0;
	MR_Info("%s: mmc_remove_host\n", __func__);
...
void resetHif_SdioAddHost(void)
{
	...
	if (isSdioAdded) {
		MR_Info("%s: sdio is already added\n", __func__);
		return;
	}
	prSdioHost->rescan_entered = 0;
	MR_Info("%s: mmc_add_host\n", __func__);
...
void resetHif_ResetGpioPull(void)
{
	...
	if (!isResetGpioReleased) {
		MR_Info("%s: reset gpio is already pulled\n", __func__);
		return;
	}
	if (!resetGpioInfo.flag_inited) {
		MR_Info("%s: reset gpio is unknown\n", __func__);
		return;
	}
	...
	if (i4Status < 0) {
		MR_Info("%s: gpio %d set output %d failed, ret = %d\n",
		       __func__, resetGpioInfo.gpio_num,
		       resetGpioInfo.action_level, i4Status);
		return;
	}

	MR_Info("%s: pull reset gpio (%d, %d)\n", __func__,
...
void resetHif_ResetGpioRelease(void)
{
	...
	if (isResetGpioReleased) {
		MR_Info("%s: reset gpio is already released\n", __func__);
		return;
	}
	if (!resetGpioInfo.flag_inited) {
		MR_Info("%s: reset gpio is unknown\n", __func__);
	...
	MR_Info("%s: release reset gpio (%d, %d)\n", __func__,
...
void resetHif_PowerGpioSwitchOn(void)
{
	...
	if (isPowerSwitchOn) {
		MR_Info("%s: power is already switched on\n", __func__);
		return;
	}
	if (!powerGpioInfo.flag_inited) {
		MR_Info("%s: power gpio is unknown\n", __func__);
		return;
	}
...
void resetHif_PowerGpioSwitchOff(void)
{
	...
	if (!isPowerSwitchOn) {
		MR_Info("%s: power is already switched off\n", __func__);
		return;
	}
	if (!powerGpioInfo.flag_inited) {
		MR_Info("%s: power gpio is unknown\n", __func__);
	...
	MR_Info("%s: power switch off (%d, %d)\n", __func__,
