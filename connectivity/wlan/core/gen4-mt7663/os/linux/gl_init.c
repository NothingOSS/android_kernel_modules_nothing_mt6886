...
struct delayed_work reg_set_workq;
...
int kalDcSetWow(void)
{
	...
	wlanSetSuspendMode(prGlueInfo, 1);
	p2pSetSuspendMode(prGlueInfo, 1);
...
void wlanRegSetWorkQueue(struct work_struct *work)
{
	...
	mtk_reg_notify(priv_to_wiphy(prGlueInfo), &request);
}
...
static int wlanInit(struct net_device *prDev)
{
	...
	INIT_DELAYED_WORK(&reg_set_workq, wlanRegSetWorkQueue);
...
static void wlanNetUnregister(struct wireless_dev *prWdev)
{
	...
	if (prWdev->netdev &&
	     prWdev->netdev->reg_state == NETREG_REGISTERED) {
		wlanClearDevIdx(prWdev->netdev);
...
{
	return atomic_read(&g_wlanProbing) ? TRUE : FALSE;
}

u_int8_t wlanIsRemoving(void)
{
	return atomic_read(&g_wlanRemoving) ? TRUE : FALSE;
}
...
static int32_t wlanProbe(void *pvData, void *pvDriverData)
{
	...
	enum ENUM_PROBE_FAIL_REASON {
		...
		BUS_SET_IRQ_FAIL,
		ADAPTER_START_FAIL,
		MAIN_TREAD_CREATE_FAIL,
		NON_MAIN_TREAD_CREATE_FAIL,
		NET_REGISTER_FAIL,
		PROC_INIT_FAIL,
	...
	if (atomic_read(&g_wlanProbing))
		return 0;
	atomic_set(&g_wlanProbing, 1);
	...
	do {
		...
		if (IS_ERR(prGlueInfo->main_thread)) {
			DBGLOG(INIT, ERROR,
			       "fail to create and wake main_thread\n");
			i4Status = -1;
			eFailReason = MAIN_TREAD_CREATE_FAIL;
			break;
		}
		...
		if (IS_ERR(prGlueInfo->hif_thread)) {
			DBGLOG(INIT, ERROR,
			       "fail to create and wake hif_thread\n");
			i4Status = -1;
			eFailReason = NON_MAIN_TREAD_CREATE_FAIL;
			break;
		}
		...
		if (IS_ERR(prGlueInfo->rx_thread)) {
			DBGLOG(INIT, ERROR,
			       "fail to create and wake rx_thread\n");
			i4Status = -1;
			eFailReason = NON_MAIN_TREAD_CREATE_FAIL;
			break;
		}
		...
		if (IS_ERR(prGlueInfo->rx_DeAgg_thread)) {
			DBGLOG(INIT, ERROR,
			       "fail to create and wake rx_DeAgg_thread\n");
			i4Status = -1;
			eFailReason = NON_MAIN_TREAD_CREATE_FAIL;
			break;
		}
	...
	if (i4Status == 0) {
		...
		case NON_MAIN_TREAD_CREATE_FAIL:
...
static void wlanRemove(void)
{
	...
	flush_delayed_work(&reg_set_workq);
	...
	wlanNetUnregister(gprWdev);

	/* 4 <7> Destroy the device */
	wlanNetDestroy(gprWdev);
	prDev = NULL;
